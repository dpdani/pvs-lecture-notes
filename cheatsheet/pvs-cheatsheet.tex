% from https://wch.github.io/latexsheet/, by Winston Chang
...
\section{Constants}
\begin{tabular}{@{}ll@{}}
	\texttt{some: int} & An undefined integer constant \\
	\texttt{some: int = 10} & The integer number 10 \\
	\texttt{abs(x: int): nat = if x >= 0 then x else -x endif} & Function definition \\
\end{tabular}
...
\section{Types}
\begin{tabular}{@{}ll@{}}
	\texttt{foo: TYPE} & Uninterpreted type \\
	\texttt{spam: TYPE+} & Non-empty type \\
	\texttt{positive\_int: TYPE = \{x: integer | x > 0\}} & Subtype \\
	\texttt{bin\_int\_f: TYPE = [int, int -> int]} & Function type \\
	\texttt{bin\_int\_f: TYPE = ARRAY[int, int -> int]} & Equivalent to above \\
	\texttt{tup: TYPE = [int, bool, [int -> int]]} & Tuple type \\
	\texttt{currency: TYPE = \{USD, EUR, JPY\}} & Enumeration type \\
	\texttt{value: TYPE = [\# amount: nat, curr: currency \#]} & Record type (struct) \\
\end{tabular}

\subsection{Recursive and higher-order types}
...
\end{verbatim}
The recursion is the \texttt{lst ...: lst?} part of the datatype definition.
...
\section{Prover commands}
\begin{tabular}{@{}ll@{}}
	\emph{Control} &\\
	\texttt{(quit)} & Close prover session\\
	\texttt{(postpone)} & Defer evaluation of this proof branch\\
	\texttt{(undo)} & Revert the changes of the previous command\\
	\texttt{(help)} & General help\\
	\texttt{(help \emph{command})} & Help on a specific command\\
